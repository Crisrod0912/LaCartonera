const LocalService = require('../services/LocalesService');
...
class LocalesController {
    ...
    async createLocales(req, res) {
        ...
        try {
            const Locales = await LocalService.createLocales(req.body);
    ...
    async getLocales(req, res) {
        ...
        try {
            const Locales = await LocalService.getLocales(req.params.id);
            if (!Locales) {
    ...
    async getLocalesWithResennas(req, res) {

        try {
            const local = await LocalService.getLocalesWithResennas(req.params.id);
            console.log(local);
    ...
    async getLocalWithReservas(req, res) {
        ...
        try {
            const local = await LocalService.getLocalWithReservas(req.params.id);
            if (!local || local.length === 0) {
                return res.status(404).json({ error: 'Local not found' });
            }
            res.json(local[0]);
